...
(function($) {
    ...
    function ajaxInit(e, app) {
        ...
        app.callbacks        = {
            ...
            {
            //    Ladda.stopAll();
                $('.block-msg-default').html("Complete!");
                setTimeout(function()
               {
                   $(el).unblock();
               }, 1000);
        ...
        app.setAjaxFormHandler = function(selector, ajaxFormSelector)
        {
            ...
            var ajaxCall = app.ajaxCaller({
               ...
               // getData: function(el) { return $(el).serialize(); }
                getData: function(el) { var $formData = new FormData(el); return $formData; }
            });
        ...
        app.ajaxCaller = function(helper) {
            return function() {
                ...
                // Use $.ajax instead so we can pass processDAta and contentType
                // This is so that the above FormData will not be converted to a string.
                // It allows file uploads to work alongside regular post data in ajax
                // forms.
                $.ajax({
                    type: "POST",
                    url: helper.getUrl(self),
                    data: helper.getData(self),
                    processData: false,
                    contentType: false
                })
