...
<hr/>
<center><h3><b> ** Please confirm the dates listed below by clicking the following link: <a style='color:red; font-weight:bold;' href='" . route('schedule_confirmation', ['id' => $job->id]) . "'>CONFIRM SCHEDULE</a>.</b> ** </h3></center>
<hr/>
<br/><br/>
...
foreach ($job->schedules()->orderBy('start', 'ASC')->get() AS $schedule)
{
    ...
    if (\Carbon\Carbon::parse($schedule->start)->hour <= 6)
    {
        $expected .= $note;
    }
    ...
    $group = (!$schedule->group) ? "Frugal Kitchens Contractor" : $schedule->group->name;
    $data .= "
  <tr>
  <td>" . \Carbon\Carbon::parse($schedule->start)->format('m/d/y h:i a') . " - " . \Carbon\Carbon::parse($schedule->end)->format('m/d/y h:i a') . "</td>
  <td>" . $group . "</td>
  <td>$expected</td>
  </tr>";
}
