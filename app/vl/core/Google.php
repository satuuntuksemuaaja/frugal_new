...
class Google
{
    ...
    static public function getClient($user = null)
    {
        ...
        try
        {
            if ($token)
            {
                $client->setAccessToken($token); //dd($client);
                if ($client->isAccessTokenExpired())
                {
                    $token = json_decode($user->google_token, true); //Get the token stored, and convert JSON to array
    ...
    static public function event($user, $params)
    {
        ...
        if (!$user) return true;
        if (!$user->google_token) return true;
