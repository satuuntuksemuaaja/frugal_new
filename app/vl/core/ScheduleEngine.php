...
class ScheduleEngine
{
    ...
    static public function send(JobSchedule $schedule)
    {
        ...
        $perDesignation .= self::getAddons($schedule, $schedule->group_id);
...
<p>$schedule->notes</p>";
    ...
    {
        // Get questions and answers for the group.
        $data = null;
        $quote = $schedule->job->quote;
        $meta = unserialize($quote->meta)['meta'];
        $object = QuoteGeneratorNew::getQuoteObject($quote);
        if (!$contract)
        {
            foreach ($object->processedQuestions AS $question => $answer)
            {
                if (($answer->answer > 0 || $answer->answer == 'Y') && $answer->question->group_id == $schedule->group_id)
                {
        ...
        else
        {
            ...
            foreach ($questions AS $question)
            {
                ...
                {
                    if ($answer->question_id == $question->id && $answer->question->group_id == $schedule->group_id &&
        ...
        if ($schedule->group_id == 2) // Electrician specific
        {
    ...
    static public function cabinets(JobSchedule $schedule)
    {
        ...
        {
            if ($addon->addon && $addon->addon->group_id == 4)
            {
    ...
    static public function plumber(JobSchedule $schedule)
    {
        ...
        foreach ($schedule->job->quote->addons as $addon)
        {
            if ($addon->addon && $addon->addon->group_id == 1)
            {
                $apps .= "<b>Addon: </b>" . sprintf($addon->addon->contract, $addon->qty,
                        $addon->description) . "<br/>";
            }
    ...
    static public function flooring(JobSchedule $schedule)
    {
        ...
        foreach ($schedule->job->quote->addons as $addon)
        {
            if ($addon->addon && $addon->addon->group_id == 11)
            {
    ...
    static public function electrician(JobSchedule $schedule)
    {
        ...
        foreach ($schedule->job->quote->addons as $addon)
        {
            if ($addon->addon && $addon->addon->group_id == 2)
            {
                $apps .= "<b>Addon: </b>" . sprintf($addon->addon->contract, $addon->qty,
                        $addon->description) . "<br/>";
            }
    ...
    static public function granite(JobSchedule $schedule, $first = false)
    {
        ...
        foreach ($schedule->job->quote->addons as $addon)
        {
            if ($addon->addon && $addon->addon->group_id == 3)
            {
...
Customer: $customer->name\n";
    ...
    static public function sendEmail(JobSchedule $schedule, $data)
    {
        ...
        Mail::send('emails.contractorSchedule', $out, function ($message) use ($user, $subject, $schedule) {
            $attached = File::whereQuoteId($schedule->job->quote->id)->whereAttached(true)->get();
    ...
    static public function getAddons($schedule, $group)
    {
        $data = "\n\n";
        foreach ($schedule->job->quote->addons as $addon)
        {
            if ($addon->addon->group_id == $group)
            {
