...
class NotificationEngine
{
    ...
    static public function fireNotification(Notification $notification)
    {
        ...
        $expiration = StatusExpiration::find($notification->expiration_id);
    ...
    static public function checkNoteNotifications(Notification $notification, Lead $lead)
    {
        $last = $lead->last_note;
        $expiration = StatusExpiration::find($notification->expiration_id);
        // Log::info("Checking StatusExpiration.. (Last: $lead->last_note) Notification exp: $notification->expires");
        if ($last < $notification->expires) return; // Don't fire this. StatusExpiration is in future.
        //  Log::info("Passed StatusExpiration Test..");
