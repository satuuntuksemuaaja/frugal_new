...
use FK3\Models\Setting as SettingModel;
use Setting;
...
class QuoteGenerator
{
    ...
    {
        $data = null;
        $quote = Quote::leftJoin('leads', 'quotes.lead_id', '=', 'leads.id')
        ...
        if (isset($obj->GTTL))
        {
            if ($quoteType->name == 'Full Kitchen' && $obj->GTTL < 50)
            {
        ...
        if ($quoteType->name != 'Granite Only')
        {
            $quoteCabinets = QuoteCabinet::leftJoin('cabinets', 'quote_cabinets.cabinet_id', '=', 'cabinets.id')
                                          ...
                                          ->leftJoin('quotes', 'quote_cabinets.quote_id', '=', 'quotes.id')
                                          ->leftJoin('leads', 'quotes.lead_id', '=', 'leads.id')
                                          ->select(
                                                    'quote_cabinets.*',
                                                    'cabinets.name as cabinetName',
                                                    'cabinets.frugal_name as cabinetFrugalName',
                                                    'cabinets.vendor_id as cabinetsVendorId',
                                                    'vendors.id as vendorId',
                                                    'vendors.colors as vendorColors',
                                                    'quotes.id as quotesId',
                                                    'leads.id as leadsId'
                                                  )
                                          ->where('quote_cabinets.quote_id', $quote->id)
                                          ->get();

            foreach ($quoteCabinets AS $cabinet)
            {
                $cab = $cabinet->cabinetFrugalName ? $cabinet->cabinetFrugalName : "Select Cabinet";
                ...
                $colorPulse = ($cabinet->vendorColors && !$cabinet->color) ? "class='pulse-red'" : null;
        ...
        if (!$quote->showroomName && $quote->status_id != 28)
        {
    ...
    public static function getEdgeCostStatic($edge)
    {
        switch ($edge)
        {
            case '(Premium) Half Bull Nose ($8/lnft.)':
    ...
    public static function evaluateAnswerStatic($quote_id, $question_id)
    {
        $answer = QuoteQuestionAnswer::where('quote_id', $quote_id)
                                      ->where('question_id', $question_id)
                                      ->first();

        $condition = QuoteQuestionCondition::where('question_id', $question_id)->first();
    ...
    public static function getSettingStatic($var)
    {
        $setting = SettingModel::whereName($var)->first();
        if (!$setting)
        {
            $s = new SettingModel();
            $s->name = $var;
    ...
    public static function checkCustomerSupplyingAppliancesPlumberStatic($meta, $forPlumber)
    {
    ...
    public static function checkCustomerSupplyingAppliancesElectricianStatic($meta, $forElectrician)
    {
        $appids = (isset($meta['quote_appliances'])) ? $meta['quote_appliances'] : [];
        $field = null;
        $appTTL = 0;
     ...
     * @return [type] [description]
     */
    public static function getTotalAmountFromWoodItemsStatic($cabinets)
    {
        $total = 0;
        // #179 - Add Wood Products if they exist to Cabinet Items.
    ...
    public static function refactorBasedonItemsStatic($quote_id)
    {
        $quote = Quote::find($quote_id);
        $cabinets = QuoteCabinet::where('quote_id', $quote->id)->get();
        // #179 - Add Wood Products if they exist to Cabinet Items.
        foreach ($cabinets AS $cabinet)
        {
            if ($cabinet->wood_xml)
        ...
        */
        ...
        // For Designer
        $quoteType = QuoteType::find($quote->quote_type_id);
        if ($quoteType->name == 'Full Kitchen' || $quoteType->name == 'Cabinet and Install')
        {
            if ($this->cabItems <= 35)
            {
                $this->forDesigner = $this->getSetting('dL35');
        ...
        {
            if ($quoteType->name == 'Cabinet Small Job' || $quoteType->name == 'Builder')
            {
                $this->forDesigner = 250;
            }
        }
        ...
        // For Frugal + on Quote and remove plumber and electrician rates.
        if ($quoteType->name == 'Cabinet Only')
        {
            foreach ($cabinets AS $cabinet)
            {
                $cabinetModel = Cabinet::join('vendors', 'cabinets.vendor_id', '=', 'vendors.id')
                                        ->select(
                                                  'cabinets.*',
                                                  'vendor.name as vendorName',
                                                  'vendor.multiplier as vendorMultiplier'
                                                )
                                        ->where('cabinets.id', $cabinet->cabinet_id)
                                        ->first();

                if (!$cabinetModel || !$cabinetModel->vendorName)
                {
                    //echo \BS::alert("danger", "Unable To Determine Cabinets",
                        //"Cabinet type has not been selected! Unable to figure multiplier.");
                    return;
                }
                $add = ($cabinet->price * $cabinetModel->vendorMultiplier) * .40;
                $this->forFrugal += $add; // Frugal gets 40% of the cabprice
                $this->setDebug("40% of {$cabinet->cabinet->frugal_name} to Frugal", $add);
                $amt = ($cabinet->measure) ? 500 : 250;
                $this->forDesigner += $amt;
    ...
    public static function buildCabinetItemsStatic($quote_id)
    {
        $rows = [];
        $headers = ['Cabinet Item List', null];
        $this->instItems = 0; // Installer Items
        ...
        $cabinets = QuoteCabinet::where('quote_id', $quote_id)->get();
        foreach ($cabinets AS $cabinet)
        {
            $cabdata = unserialize($cabinet->data);
            if (!is_array($cabdata))
            ...
            foreach ($cabdata AS $item)
            {
                ...
                {
                    $objItems = array();
                    $objItems[] = '<span class="color-info">('. $item[sku] . ' - ' . $item[description] . ')</span>';
                    $objItems[] = "$" . number_format($item['price'], 2) . " x " . $item['qty'] . " : QTY: " . $item['qty'];
                    $rows[] = $objItems;
                ...
                {
                    $objItems = array();
                    $objItems[] = '<span class="color-info">Attachment: (' . $item[sku] . ' - ' . $item[description] . ') </span>';
                    $objItems[] = "$" . number_format($item['price'], 2) . " x " . $item['qty'] . ": QTY: " . $item['qty'];
                    $rows[] = $objItems;
    ...
    public function determineValidQuestion(QuoteAnswer $answer, Quote $quote, $force = false)
    {
        ...
        if (array_key_exists($question->id, $this->processedQuestions))
        {
            if ($force)
            {
                $this->processedQuestions[$answer->question_id] = $answer;
    ...
    public static function contractorAdjustmentBuilderStatic($quote, $force = false)
    {
        if (!$quote)
        {
            return;
        ...
        $answers = QuoteQuestionAnswer::leftJoin('quote_questions', 'quote_question_answers.question_id', '=', 'quote_questions.id')
                                        ->select(
                                                  'quote_question_answers.*',
                                                  'quote_questions.question as questionsQuestion',
                                                  'quote_questions.active as questionsActive'
                                                )
                                        ->where('quote_question_answers.quote_id', $quote->id)
                                        ->get();
        foreach ($answers AS $answer)
        {
            ...
            if (!$answer->questionsQuestion || !$answer->questionsActive)
            {
                continue;
            }

            if (!$answer->questionsQuestion)
            {
                $answer->delete();
                continue;
            }
            ...
            $answerModel = QuoteQuestionAnswer::find($answer->id);
            $this->determineValidQuestion($answerModel, $quote, $force);
        } // main FE loop.
    ...
    public static function processContractorAdjustmentsStatic()
    {
        // If this is NOT the final quote we need to work with then...
        foreach ($this->processedQuestions AS $question => $answer)
        {
            ...
            if ($value != 0)
            {
                ...
                $objItems = array();
                $objItems[] = '<span class="color-info">' . $question->question . '</a>';
                $objItems[] = "<a href='/quote/{$this->quote->id}/questions'>" . $answer->answer . "</a>";
                $objItems[] = ($this->admin) ? "$" . number_format($value, 2) : null;
                $objItems[] = ($this->admin) ? "$" . number_format($this->total, 2) : null;
...
<b>Buyers Signature__________________________________ &nbsp; &nbsp; &nbsp;    Date: _______________________</b>";
     ...
     * @return mixed
     */
    public static function checkPromoStatic(Promotion $promotion = null, $amount)
    {
        if (!$promotion) return $amount;
        // By default we will return the same amount that was given.
        // Since we don't know what a promotion is supposed to do.
